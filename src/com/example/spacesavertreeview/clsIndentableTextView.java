...
public class clsIndentableTextView extends TextView  {
	...
	private void init() {
		...
		/*
		 * 
		 * float[] mat = new float[] { -1, 0, 0, 0, 255, 0, -1, 0, 0, 255, 0, 0,
		 * -1, 0, 255, 0, 0, 0, 1, 0 }; cf = new ColorMatrixColorFilter(new
		 * ColorMatrix(mat));
		 * 
		 * paintTextWhite = new Paint(); paintTextWhite.setDither(true);
		 ...
		 * 
		 * paintTextBlack = new Paint(); paintTextBlack.setDither(true);
		 ...
		 * PorterDuffColorFilter(Color.BLACK, PorterDuff.Mode.OVERLAY);
		 */
		...
		float fltLineWidthInDp = clsUtils.dpToPx(getContext(),
				getResources().getInteger(R.integer.select_line_width_in_dp));
	...
	protected void onDraw(Canvas canvas) {
		...
		objRectMyViewHeightAdjusted = new Rect(objRectMyView.left, objRectMyView.top, objRectMyView.right, Math.max(
				objRectMyView.bottom, objRectMyView.top + GetThumbnailHeightInPx()));
		...
		int intTextYpos = (int) (((objRectMyViewHeightAdjusted.bottom - objRectMyViewHeightAdjusted.top) / 2) - ((paintPlainBlack
		...
		myLinearGradient = new LinearGradient(objIndentRect.left, objIndentRect.top, objIndentRect.right,
				objIndentRect.bottom, intColorStart, intColorEnd, Shader.TileMode.CLAMP);
	...
	private void DrawCustomRect(Canvas objCanvas, RectF objRect, Paint objPaint) {
		...
		if (_objListItem.GetAboveItemLevelRelation() == enumItemLevelRelation.SAME) {
			if (_objListItem.GetBelowItemLevelRelation() == enumItemLevelRelation.SAME) {
				...
				path.addArc(new RectF(objRect.left, objRect.bottom - fltDiam, objRect.left + fltDiam, objRect.bottom),
						90, 90);
				...
				path.lineTo(objRect.right, objRect.bottom);
				path.lineTo(objRect.left - fltRadius, objRect.bottom);
				path.addArc(new RectF(objRect.left - fltDiam, objRect.bottom - fltDiam, objRect.left, objRect.bottom),
						90, -90);
				path.lineTo(objRect.left, objRect.top);
				path.lineTo(objRect.right, objRect.top);
				objCanvas.drawPath(path, objPaint);
			...
			if (_objListItem.GetBelowItemLevelRelation() == enumItemLevelRelation.SAME) {
				...
				path.addArc(new RectF(objRect.left, objRect.top, objRect.left + fltDiam, objRect.top + fltDiam), 180,
						90);
				path.lineTo(objRect.right, objRect.top);
				objCanvas.drawPath(path, objPaint);
			} else if (_objListItem.GetBelowItemLevelRelation() == enumItemLevelRelation.HIGHER) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// getMeasuredHeight() and getMeasuredWidth() now contain the suggested
		// size
	}
